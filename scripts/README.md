...
twilight times for today. Coordinates are given in decimal degrees, for example `npx ts-node rise-set.ts 34.05 -118.25` gives data 
...
longitude coordinates. For example, `npx ts-node rise-set.ts 40.75 -73.99 2025-12-31T16:15:00` gives the sun position and distance 
for Manhattan, New York City on December 31, 2025 at 16:15 (4:15 pm) eastern standard time, along with sunrise, sunset, and twilight 
times for December 31 in Manhattan.

`npx ts-node scripts/rise-set.ts <latitude> <longitude> <me/js/se/ds>` gives sun position and sunrise/sunset/twilight data for the 
given latitude and longitude at the March equinox (me), June solstice (js), September equinox (se), or December solstice (ds).
...
`npx ts-node scripts/solstice.ts <year>`: Prints the times of solstices and equinoxes in the given year, in the device's local time 
zone, along with subsolar points.

`npx ts-node scripts/solstice.ts <year> <time zone>`: Prints the times of solstices and equinoxes in the given year and time zone. 
The time zones are either in IANA time zone format (ex. "America/Los_Angeles" for Pacific Time) or a fixed UTC offset (ex. "utc", "utc-8", "utc+5:30").

## gen-solsteq-json.ts

`npx ts-node scripts/gen-solsteq-json.ts`: Generates a JSON file containing solstice and equinox times in UTC for all years from 0 to 
2500.
...
## moonrise-set.ts
`npx ts-node scripts/moonrise-set.ts <latitude> <longitude>`: Prints the times of moonrise and moonset along with other lunar data.