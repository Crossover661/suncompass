...
function solarMidnight(longitude, date) {
    ...
    midnight = midnight.minus({milliseconds: 1}); // In some places (such as Lebanon), daylight saving time adjustments occur at midnight, so the day might not necessarily start at midnight. To make up for this, the clock is set to 23:59:59.999 on the previous day
...
function lowestSunAngle(lat, long, date) { // See description for highestSunAngle(). The return value for this function is the same as solarMidnight if the latitude is between -88 and 88.
...
function dawn(lat, long, date, angle) {
    ...
    if (Math.abs(lat) <= 88-Math.abs(declination(noon))-Math.abs(angle)) { // calculate based on the sunrise equation
        ...
        if (sunPosition(lat, long, noon)[0] <= angle) {return Number.NEGATIVE_INFINITY;} // polar night
        else if (sunPosition(lat, long, midnight)[0] >= angle) {return Number.POSITIVE_INFINITY;} // midnight sun
        else if (midnight > noon) {return NaN;} // In this case, there cannot be a sunrise. This occurs at the poles around the fall equinox when the sun sets.
        dawn = midnight;
        var t1 = 0;
...
function dusk(lat, long, date, angle) {
    ...
    if (Math.abs(lat) <= 88-Math.abs(declination(noon))-Math.abs(angle)) {
        ...
        else if (noon > midnight) {return NaN;}
        dusk = noon;
        var t1 = 0;
...
function dayLength(lat, long, date) { 
    // Returns length of daylight as a DateTime duration (hours, minutes, seconds), negative infinity in polar night, or positive infinity in midnight sun.
    var rise = sunrise(lat, long, date);
    var set = sunset(lat, long, date);
    if (rise == Number.NEGATIVE_INFINITY || set == Number.NEGATIVE_INFINITY) {return Number.NEGATIVE_INFINITY;}
    else if (rise == Number.POSITIVE_INFINITY || set == Number.POSITIVE_INFINITY) {return Number.POSITIVE_INFINITY;}
    else if (isNaN(rise) || isNaN(set)) {return NaN;}
    else { 
        // If the sun rises and sets. This is always the case in latitudes within (90-axialTilt-5/6) degrees of the equator.
        var length = set.diff(rise, ["hours", "minutes", "seconds"]);
        return length;
    }
}
