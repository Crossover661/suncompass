...
import { generateLODProfile, estimateLOD } from "./lookup-tables.js";
...
export function sunMeanLong(date, unix = false) {
    if (!unix) { // if date is specified as a Julian century
        ...
        return sunMeanLong(mf.jCentury(date));
    }
}
...
export function sunGeomLong(date, unix = false) {
    if (!unix) {
        ...
        return sunGeomLong(mf.jCentury(date));
    }
}
/** Formula 45.3, in page 308 of Astronomical Algorithms */
export function meanSunAnomaly(JC) { return mf.mod(357.5291092 + 35999.0502909 * JC - 1.536e-4 * JC ** 2 + JC ** 3 / 24490000, 360); }
...
export function sunDistance(date, unix = false) {
    if (typeof (date) == "number") {
        ...
        return sunDistance(mf.jCentury(mf.ms(date)));
    }
}
/**
...
export function sunTrueLong(date, unix = false) {
    if (!unix) {
        ...
        return sunTrueLong(mf.jCentury(date));
    }
}
...
export function obNutation(date, unix = false) {
    if (!unix) {
        ...
        return obNutation(mf.jCentury(date));
    }
}
...
export function longNutation(date, unix = false) {
    if (!unix) {
        ...
        return longNutation(mf.jCentury(date));
    }
}
/**
...
export function obliquity(date, unix = false) {
    if (!unix) {
        ...
        return obliquity(mf.jCentury(date));
    }
}
/**
...
export function equationOfTime(date, unix = false) {
    if (!unix) {
        ...
        return equationOfTime(mf.jCentury(date));
    }
}
 ...
 */
export function solarNoon(lat, long, start, end, ecef) {
 ...
 */
export function solarMidnight(lat, long, start, end, ecef) {
    const startT = start.unix, endT = end.unix;
    const st00 = solarTime(long, startT), st24 = solarTime(long, endT);
    const stDiff = mf.mod(st24 - st00 - 720, 1440) + 720;
    const solarTimeRate = stDiff / (endT - startT) * 60000; // rate of solar time change relative to actual time
 ...
 */
export function declination(longitude, obliquity) {
 ...
 */
export function subsolarPoint(lod) {
 ...
 */
export function sunPosition(lat, long, lod, ecefO) {
    ...
    if (ecefO === undefined) {
        ...
        return mf.elevAzimuth(lat, long, ecefO, sunEcef);
    }
}
/**
...
export function derivative(lat, long, unix, startLOD, endLOD, ecef) {
 ...
 */
export function dawn(lat, long, angle, type, ecef, maxMin, startLOD, endLOD) {
    const dawnTimes = [];
    for (let i = 0; i < maxMin.length - 1; i++) {
        ...
        if (s0[0] <= angle && s1[0] >= angle) {
            while (t1 - t0 > 1) {
                ...
                if (sunPosition(lat, long, avgLOD, ecef)[0] < angle) {
                    t0 = avgLOD.unix;
                }
                else {
                    t1 = avgLOD.unix;
 ...
 */
export function dusk(lat, long, angle, type, ecef, maxMin, startLOD, endLOD) {
    const duskTimes = [];
    for (let i = 0; i < maxMin.length - 1; i++) {
        let t0 = maxMin[i], t1 = maxMin[i + 1];
        const lod0 = estimateLOD(t0, startLOD, endLOD), lod1 = estimateLOD(t1, startLOD, endLOD);
...
export function sunrise(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dawn(lat, long, -5 / 6, "Sunrise", ecef, maxMin, startLOD, endLOD);
}
export function sunset(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dusk(lat, long, -5 / 6, "Sunset", ecef, maxMin, startLOD, endLOD);
}
export function civilDawn(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dawn(lat, long, -6, "Civil Dawn", ecef, maxMin, startLOD, endLOD);
}
export function civilDusk(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dusk(lat, long, -6, "Civil Dusk", ecef, maxMin, startLOD, endLOD);
}
export function nauticalDawn(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dawn(lat, long, -12, "Nautical Dawn", ecef, maxMin, startLOD, endLOD);
}
export function nauticalDusk(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dusk(lat, long, -12, "Nautical Dusk", ecef, maxMin, startLOD, endLOD);
}
export function astroDawn(lat, long, ecef, maxMin, startLOD, endLOD) {
    return dawn(lat, long, -18, "Astro Dawn", ecef, maxMin, startLOD, endLOD);
}
export function astroDusk(lat, long, ecef, maxMin, startLOD, endLOD) {
...
export function allSunEvents(lat, long, start, end, ecef) {
    if (ecef === undefined) {
        ecef = mf.latLongEcef(lat, long);
    }
 ...
 */
export function intervals(sunEvents, timeZone) {
    ...
    if (etype == "Sunset") {
        ints[0].push([0, newSunEvents[0].timeOfDay(timeZone)]);
    }
    else if (etype == "Sunrise" || etype == "Civil Dusk") {
        ints[1].push([0, newSunEvents[0].timeOfDay(timeZone)]);
    }
    else if (etype == "Civil Dawn" || etype == "Nautical Dusk") {
        ints[2].push([0, newSunEvents[0].timeOfDay(timeZone)]);
    }
    else if (etype == "Nautical Dawn" || etype == "Astro Dusk") {
        ints[3].push([0, newSunEvents[0].timeOfDay(timeZone)]);
    }
    else if (etype == "Astro Dawn") {
        ints[4].push([0, newSunEvents[0].timeOfDay(timeZone)]);
    }
 ...
 */
export function intervalsSvg(sunEvents, timeZone) {
    const newSunEvents = []; // sunEvents without solar noon or midnight
    for (const event of sunEvents) {
        if (event.eventType != "Solar Noon" && event.eventType != "Solar Midnight") {
            newSunEvents.push(event);
 ...
 */
export function lengths(sunEvents, timeZone) {
    const newSunEvents = []; // sunEvents without solar noon or midnight
