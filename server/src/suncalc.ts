...
import {clamp, mod, jCentury, approxDeltaT, startOfDay, startNextDay, convertToMS, ms, jdUTC, toEcef, elevAzimuth, latLongEcef} from "./mathfuncs.js";
import {degToRad, sunPeriodicTerms, DAY_LENGTH} from "./constants.js";
import * as fs from "fs";
import SunTime from "./SunTime.js";
import {DateTime} from "luxon";
import { NumericLiteral } from "../../node_modules/typescript/lib/typescript.js";
...
export function obNutation(date: number, unix = false) {
    ...
    else {return obNutation(jCentury(date));}
}
...
export function longNutation(date: number, unix = false) {
    ...
    else {return longNutation(jCentury(date));}
}
...
export function gast(unix: number): number {
    const JD = jdUTC(unix); // Julian day but using UTC (approximation to UT1) rather than TT
 ...
 * @param unix Unix timestamp in milliseconds.
 */
export function sunAngularRadius(unix: number): number {
    return (695700 / sunDistance(unix, true)) / degToRad;
}
...
export function solarTime(longitude: number, unix: number): number {
    ...
    const mst = mod(unix / 60000, 1440) + longitude * 4; // mean solar time in minutes
    const ast = mod(timeEq + mst, 1440); // apparent solar time
    return ast;
}
 ...
 */
export function solarNoon(lat: number, long: number, start: number, end: number, ecef: number[], timeZone: string): SunTime[] {
    const dayLength = (end - start) / 60000; // usually 1440, can be 1380 or 1500 with DST
    ...
    if (st00 > 600 && st00 <= 720 && st24 > 720 && st24 < 840) { // 2 solar noons in a day
        ...
        if (solarNoon0 < start) {solarNoon0 = start;}
        solarNoon0 = Math.floor(solarNoon0);
        ...
        if (solarNoon1 >= end) {solarNoon1 = end - 1;}
        solarNoon1 = Math.floor(solarNoon1);
        const [e1, a1] = sunPosition(lat, long, solarNoon1, ecef); // solar elevation/azimuth at solarNoon1
        return [new SunTime(DateTime.fromMillis(solarNoon0, {zone: timeZone}), e0, a0, "Solar Noon"), 
            new SunTime(DateTime.fromMillis(solarNoon1, {zone: timeZone}), e1, a1, "Solar Noon")];
    }
    else if (st00 > 720 && st00 < 840 && st24 > 600 && st24 <= 720) { // 0 solar noons in a day
        return [];
    }
    else { // 1 solar noon in a day
        let solarNoon = start + (mod(720 - st00, 1440) / solarTimeRate) * 60000;
        solarNoon += ((720 - solarTime(long, solarNoon)) * 60000);
        if (solarNoon < start) {solarNoon = start;}
        else if (solarNoon >= end) {solarNoon = end - 1;}
        solarNoon = Math.floor(solarNoon);
        const [e, a] = sunPosition(lat, long, solarNoon, ecef);
        return [new SunTime(DateTime.fromMillis(solarNoon, {zone: timeZone}), e, a, "Solar Noon")];
    }
}
 ...
 */
export function solarMidnight(lat: number, long: number, start: number, end: number, ecef: number[], timeZone: string): SunTime[] {
    const dayLength = (end - start) * 60000; // usually 1440, can be 1380 or 1500 with DST
    const st00 = solarTime(long, start), st24 = solarTime(long, end);
    const stDiff = mod(st24 - st00 - 720, 1440) + 720;
    const solarTimeRate = stDiff / dayLength; // the rate at which solar time changes through the day, relative to actual time
    ...
    if (st00 > 1320 && st24 < 120) { // 2 solar midnights in a day
        ...
        solarMidnight0 += ((720-mod(solarTime(long,solarMidnight0)+720,1440))*60000);
        if (solarMidnight0 < start) {solarMidnight0 = start;}
        solarMidnight0 = Math.floor(solarMidnight0);
        ...
        solarMidnight1 += ((720-mod(solarTime(long,solarMidnight1)+720,1440))*60000);
        if (solarMidnight1 >= end) {solarMidnight1 = end - 1;}
        solarMidnight1 = Math.floor(solarMidnight1);
        const [e1, a1] = sunPosition(lat, long, solarMidnight1, ecef); // solar elevation/azimuth at solarMidnight1
        return [new SunTime(DateTime.fromMillis(solarMidnight0, {zone: timeZone}), e0, a0, "Solar Midnight"), 
            new SunTime(DateTime.fromMillis(solarMidnight1, {zone: timeZone}), e1, a1, "Solar Midnight")];
    }
        ...
        solarMidnight += ((720-mod(solarTime(long,solarMidnight)+720,1440))*60000);
        if (solarMidnight < start) {solarMidnight = start;}
        else if (solarMidnight >= end) {solarMidnight = end - 1;}
        solarMidnight = Math.floor(solarMidnight);
        const [e, a] = sunPosition(lat, long, solarMidnight, ecef);
        return [new SunTime(DateTime.fromMillis(solarMidnight, {zone: timeZone}), e, a, "Solar Midnight")];
    }
}
 ...
 */
export function subsolarPoint(unix: number = Date.now()): number[] {
    const JC = jCentury(unix);
    const subsolarLat = declination(JC);
    const soltime0 = solarTime(0, unix); // solar time at Greenwich meridian (longitude 0)
 ...
 */
export function sunPosition(lat: number, long: number, date: number, ecefO?: number[]): number[] {
    const [sunLat, sunLong] = subsolarPoint(date);
    const sunEcef = toEcef(sunLat, sunLong, sunDistance(date, true));
...
export function maxAndMin(lat: number, long: number, start: number, end: number, ecef: number[]): number[] {
    const times = [start];
    const intervals = [start,start+4*60000,start+8*60000,start+12*60000,start+16*60000,start+20*60000,end];
    for (let i=0; i<intervals.length-1; i++) {
        // use binary search to find the time closest to zero derivative
        const d0 = derivative(lat, long, intervals[i], ecef), d1 = derivative(lat, long, intervals[i+1], ecef);
 ...
 */
export function dawn(lat: number, long: number, angle: number, type: string, ecef: number[], maxMin: number[], timeZone: string): 
SunTime[] {
    const dawnTimes = [];
    for (let i=0; i<maxMin.length-1; i++) {
        ...
        if (s0[0] <= angle && s1[0] >= angle) {
            ...
            dawnTimes.push(new SunTime(DateTime.fromMillis(t0, {zone: timeZone}), elev, az, type));
        }
    }
    return dawnTimes;
}
 ...
 */
export function dusk(lat: number, long: number, angle: number, type: string, ecef: number[], maxMin: number[], timeZone: string): 
SunTime[] {
    const duskTimes = [];
    for (let i=0; i<maxMin.length-1; i++) {
        let t0 = maxMin[i], t1 = maxMin[i+1];
        ...
        if (s0 >= angle && s1 <= angle) {
            ...
            const [elev, az] = sunPosition(lat, long, t0, ecef);
            duskTimes.push(new SunTime(DateTime.fromMillis(t0, {zone: timeZone}), elev, az, type));
        }
    }
    return duskTimes;
}

export function sunrise(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dawn(lat, long, -5/6, "Sunrise", ecef, maxMin, timeZone);
} 
export function sunset(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dusk(lat, long, -5/6, "Sunset", ecef, maxMin, timeZone);
}
export function civilDawn(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dawn(lat, long, -6, "Civil Dawn", ecef, maxMin, timeZone);
}
export function civilDusk(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dusk(lat, long, -6, "Civil Dusk", ecef, maxMin, timeZone);
}
export function nauticalDawn(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dawn(lat, long, -12, "Nautical Dawn", ecef, maxMin, timeZone);
}
export function nauticalDusk(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dusk(lat, long, -12, "Nautical Dusk", ecef, maxMin, timeZone);
}
export function astroDawn(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dawn(lat, long, -18, "Astro Dawn", ecef, maxMin, timeZone);
}
export function astroDusk(lat: number, long: number, ecef: number[], maxMin: number[], timeZone: string) {
    return dusk(lat, long, -18, "Astro Dusk", ecef, maxMin, timeZone);
}
...
function allSunEvents(lat: number, long: number, start: number, end: number, timeZone: string, ecef?: number[]): SunTime[] {
    if (ecef === undefined) {ecef = latLongEcef(lat, long);}
    const maxMin = maxAndMin(lat, long, start, end, ecef);
    const midnight = solarMidnight(lat, long, start, end, ecef, timeZone);
    const adawn = astroDawn(lat, long, ecef, maxMin, timeZone);
    const ndawn = nauticalDawn(lat, long, ecef, maxMin, timeZone);
    const cdawn = civilDawn(lat, long, ecef, maxMin, timeZone);
    const rise = sunrise(lat, long, ecef, maxMin, timeZone);
    const noon = solarNoon(lat, long, start, end, ecef, timeZone);
    const set = sunset(lat, long, ecef, maxMin, timeZone);
    const cdusk = civilDusk(lat, long, ecef, maxMin, timeZone);
    const ndusk = nauticalDusk(lat, long, ecef, maxMin, timeZone);
    const adusk = astroDusk(lat, long, ecef, maxMin, timeZone);
...
export function sunEventsDay(lat: number, long: number, date: DateTime, ecef: number[]): SunTime[] {
    ...
    return allSunEvents(lat, long, start, end, zone, ecef);
}
...
export function calcSolstEq(year = DateTime.now().toUTC().year, month: number) {
    let t0 = ms(DateTime.fromObject({year:year, month:month, day:10}, {zone: "utc"}));
    let t1 = t0 + 18*86400*1000; // 18 days after start
    while (t1 - t0 > 1) {
        const avg = Math.floor((t0+t1)/2);
        if (month == 3) {(sunTrueLong(avg) >= 180) ? t0 = avg : t1 = avg;}
        else {(sunTrueLong(avg) <= 30*(month-3)) ? t0 = avg : t1 = avg;}
    }
    return t0;
}
 ...
 * @param timeZone Time zone of the given location (ex. "America/Los_Angeles")
