...
import {clamp, mod, mins, jCentury, approxDeltaT, startOfDay, startNextDay, convertToMS, ms} from "./mathfuncs.js";
...
export function declination(date: number | DateTime): number {
    if (typeof(date) == "number") {return Math.asin(clamp(Math.sin(obliquity(date)*degToRad)*Math.sin(sunTrueLong(date)*degToRad))) / degToRad;}
